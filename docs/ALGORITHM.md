...
- 底图和输出均为 DDS
- 请求的 BC 压缩格式（含 sRGB 标记）与底图一致；不一致则按用户选择全量重编码
- 最终画布尺寸与底图一致
...
布局计算与像素合成分离。普通预览保留原始坐标和画布尺寸，仅为缩小后的栅格分配内存；导出复用同一布局并输出完整分辨率。项目快照使用 `Arc<RgbaImage>` 共享不可变像素。

底图模式在前端 Canvas 上绘制缓存图像，替换槽位先清除对应区域再绘制，避免透明部分透出旧底图。画布扩展区域保持透明。空槽位以网格背景和指针坐标命中测试表示，只有已放置贴图创建交互节点，避免大网格阻塞界面。

加载任务提交前校验版本；清除操作使旧任务失效。设置只在布局变化时失效预览，背景文件读取失败不会删除已保存的路径。
